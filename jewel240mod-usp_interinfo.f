...
C--identifier of file for hepmc output and logfile
	common/hepmcid/hpmcfid,logfid,scatinfo
	integer hpmcfid,logfid,scatinfo
C--number of protons
...
C--event loop
     ...
     &b1, ' + ', b2, ' ========='
          NSCATHIGHPT = 0
	      call genevent(j,b1,b2)
          write(scatinfo,*) 'N high-pT scatterings = ', NSCATHIGHPT, 
     &', evt weight = ', EVWEIGHT
          write(scatinfo,*) 
 102	    CONTINUE
	    sumofweightstot = sumofweightstot+sumofweights
...
C--Variables local to this program
	...
	CHARACTER*2 SNSET
     ...
     &label,value
      CHARACTER*220 HEPMCFILE,LOGFILE,FILENAME2,scatinfofile
...
C--check for scattering
      NOSCAT=.NOT.GETDELTAT(L,TSTART,DELTAT,DT,"I")
...
C--find number, position and momentum transfers of further scatterings
	NOSCAT=.NOT.GETDELTAT(L,TNOW,MIN(TLEFT,TAUEST),DELTAL,"I")
...
C--check for radiation with first (given) momentum transfer
 ...
 222	IF((NEWMASS.EQ.0.d0).OR.(NEWMASS.EQ.P(L,5)))THEN
	...
	LAMBDA=1.d0/TAUEST
	TAUEST=-LOG(1.d0-RTAU)/LAMBDA
      NOSCAT=.NOT.GETDELTAT(L,TNOW+TSUM,MIN(TAUEST,TLEFT),DELTAL,"F")
	IF(.NOT.NOSCAT)THEN
C--do scattering
	 NEND=NEND+1
...
C--set the production vertices: x_mother + (tprod - tprod_mother) * beta_mother
	 ...
	 if ((.not.redokin).and.(.not.rejectt)) then
         if (oldpt.ge.minptscatinfo) NSCATHIGHPT=NSCATHIGHPT+1
      endif
...
***	  function getdeltat
***********************************************************************
      LOGICAL FUNCTION GETDELTAT(LINE,TSTART,DTMAX1,DELTAT,STATETYPE)
      IMPLICIT NONE
C--identifier of file for hepmc output and logfile
	common/hepmcid/hpmcfid,logfid,scatinfo
	integer hpmcfid,logfid,scatinfo
