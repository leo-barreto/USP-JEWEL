...
C--boost to fluid rest frame
	   ...
	   !endif
         !endif
           ...
           ! Hydro changes: rechardcut and recsoftcut are fixed 
           ! and compared to pT on lab frame
           ptcompare = sqrt(p(n-1,1)**2+p(n-1,2)**2)
	   if (ptcompare.lt.recsoftcut) then
	     softrec = .true.
	     k(n-1,1)=13
	   else
	     softrec = .false.

           if (scatrecoil.and.(ptcompare.GT.rechardcut)) THEN
	       K(N-1,1)=2
           else
             K(N-1,1)=3
           ENDIF
