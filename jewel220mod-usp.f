...
C--Variables local to this program
	...
	character firstchar
	CHARACTER*2 SNSET
      CHARACTER*100 PDFFILE,XSECFILE,FILEMED,FILESPLIT,buffer,
     &label,value
      CHARACTER*200 HEPMCFILE,LOGFILE,FILENAME2
...
C--write message to log-file
 102  IF(NSIM.GT.100)THEN
 	  ...
 	  write(*,*) 'done with event number ',J
 	 ENDIF
	else
 	  write(logfid,*) 'done with event number ',J
 	  write(*,*) 'done with event number ',J
...
C--local variables
	...
	DATA PI/3.141592653589793d0/
      ...
      !localtemp = gettemp(XDEC(1),XDEC(2),XDEC(3),MV(L,5))
      !localtime = sqrt(mv(l,5)**2-xdec(3)**2)
...
C--local variables
     ...
     &SIGMAMIN,NEFFMIN,TSTART,DTMAX1,DELTAT,GETNEFFLIMITS
	CHARACTER PTYPE
	LOGICAL STOPNOW
...
!     &	xstart,ystart,-sign(abs(zstart),p(line,3)),zstart+1.d-6)
	...
	DO 333 I=1,1000000
	 ...
	 IF(WEIGHT.GT.4.d0+1d-6) then
	   if (line.ne.errl) then
             !There are an issue with the weight when etamax approx. 1
             !Signal if weight > 4, but weight > 1 is still a rare
             !occurrence
     	     write(logfid,*)'error in GETDELTAT: weight > 4',WEIGHT,
